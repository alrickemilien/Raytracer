...
int		belong_to_box(double t, t_obj obj, t_ray *ray)
{
	...
	if (intersection.x >= -obj.size - 0.0001 && intersection.x <= obj.size + 0.0001 &&
		intersection.y >= -obj.size - 0.0001 && intersection.y <= obj.size + 0.0001 &&
		intersection.z >= -obj.size - 0.0001 && intersection.z <= obj.size + 0.0001)
		return (1);
	return (0);
}
